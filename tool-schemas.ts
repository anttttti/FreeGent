    ...
    if (hasNative) {
        ...
        if (hasWasm && hasPyodide) {
            description = 'Execute Bash, Python (Pyodide), or JavaScript (browser sandbox) in-page. ' +
                ...
                'Python: workspace files pre-loaded, writes sync back; numpy, pandas, matplotlib, Pillow, scikit-learn auto-load from imports — use micropip.install("pkg") for extras. Image files written by Python are displayed inline automatically. No subprocess, no network (use fetch_url). ' +
            ...
            description = 'Execute Python (Pyodide) or JavaScript (browser sandbox) in-page. ' +
                'Python: files pre-loaded, writes sync back; only numpy, pandas, matplotlib, Pillow, scikit-learn auto-load from imports — use micropip.install("pkg") for anything else. Image files written by Python are displayed inline automatically. No subprocess, no network (use fetch_url). ' +
            ...
            agents: {
                ...
                items: {
                    ...
                    properties: {
                        ...
                        task:  { type: 'string', description: 'Full task description including all context the worker needs to work independently.' },
                        role:  { type: 'string', enum: ['researcher', 'coder', 'director'], description: 'Role determines system prompt and tool access. researcher: read-only workspace search — list_files, read_file, repo_map, search_workspace, execute_code. coder: reads, edits, and runs code — list_files, read_file, repo_map, search_workspace, write_file, replace_in_file, apply_patch, execute_code. director: multi-step orchestration with run_workers; reads and executes — use coder for file edits.' },
