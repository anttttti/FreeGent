...
export function _buildLangsDesc(): string {
    ...
    if (_wasm && _p) return 'Languages: **bash** (musl-static Unix tools via WASM; workspace at /workspace — use absolute paths), **python** (Pyodide — writes sync back, micropip for extras), **javascript** (browser sandbox — fs.readFileSync/writeFileSync, no npm).';
